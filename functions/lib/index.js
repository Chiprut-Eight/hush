...
exports.onSecretUpdated = exports.decaySecrets = void 0;
...
exports.decaySecrets = functions.pubsub.schedule('every 24 hours').onRun(async (context) => {
    const now = new Date();
    ...
    for (const doc of snapshot.docs) {
        ...
        if (shouldDelete) {
            ...
            if (data.creatorId) {
                await sendNotification(data.creatorId, 'A secret vanished into thin air 💨', `Your secret was removed due to ${reason}. Stay active to keep your secrets alive!`);
            }
        }
    }
    if (deletedCount > 0) {
...
exports.onSecretUpdated = functions.firestore
    ...
    if (newData.likes > oldData.likes) {
        await sendNotification(creatorId, 'Someone liked your secret! ❤️', 'A user nearby discovered and liked your secret.');
    }
    ...
    if (newSaved.length > oldSaved.length) {
        await sendNotification(creatorId, 'Someone saved your secret! 🔖', 'Your secret was so intriguing that someone saved it to their private collection.');
    }
    return null;
});
